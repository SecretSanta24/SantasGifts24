...
{
    [Tracked]
    [CustomEntity("SS2024/OcularBarrier")]
    public class OcularBarrier : Solid
    {
        ...
        {
            string path = data.Attr("texturePath", "objects/ss2024/ocularBarrier/");
            flag = data.Attr("flag", "lookout_interacting");
            invert = data.Bool("invert", false);
            MTexture mTexture = GFX.Game[path + "binonineslice"];
            MTexture mTexture2 = GFX.Game[path + "binoninesliceoff"];
            centerOn = mTexture.GetSubtexture(24, 0, 10, 8);
            centerOff = mTexture2.GetSubtexture(24, 0, 10, 8);
